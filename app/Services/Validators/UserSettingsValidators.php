...
final class UserSettingsValidators
{
    /**
    ...
    public static function byKey(UserSettings|string $key, array|string $data): Validator
    {
        ...
        if (is_string($data)) {
            throw new InvalidArgumentException('Data must be an array for validation');
        }
    ...
    {
        $providerValues = implode(',', array_column(TwoFactorProvider::cases(), 'value'));
        ...
        $rules = [
            ...
            'two_factor.provider' => $providerValues !== ''
                ? 'nullable|string|in:'.$providerValues
                : 'nullable|string',
            'two_factor.secret' => 'nullable|string|required_if:two_factor.provider,'.TwoFactorProvider::AUTHENTICATOR->value,
