...
new #[Layout('components.layouts.guest')] class extends Component
{
    ...
    public function authenticate()
    {
        ...
        if (Auth::attempt(credentials: $this->only('email', 'password'), remember: $this->remember)) {
...
<div class="flex flex-col items-center justify-center px-6 py-8 mx-auto md:h-screen lg:py-0">
    <x-slot name="title">{{ __('auth.login.page_title') }}</x-slot>
    ...
    <div class="w-full bg-white rounded-lg shadow dark:border md:mt-0 sm:max-w-md xl:p-0 dark:bg-zinc-800 dark:border-zinc-700">
        <div class="p-6 space-y-4 md:space-y-6 sm:p-8">
            ...
            <form class="space-y-4 md:space-y-6" wire:submit="authenticate">
                {{-- Email --}}
                <div>
                    <label for="email" class="block mb-2 text-sm font-medium text-zinc-900 dark:text-white">{{ __('auth.login.email') }}</label>
                ...
                {{-- Password --}}
                <div>
                    <label for="password" class="block mb-2 text-sm font-medium text-zinc-900 dark:text-white">{{ __('auth.login.password') }}</label>
                ...
                <div class="flex items-center justify-between">
                    <div class="flex items-start">
                        ...
                        <div class="ml-3 text-sm">
                            <label for="remember" class="text-zinc-500 dark:text-zinc-300">{{ __('auth.login.remember') }}</label>
                        </div>
                    </div>

                    <a href="{{ route('password.forgot') }}" class="text-sm font-medium text-primary-600 hover:underline dark:text-primary-500">{{ __('auth.login.forgot_password') }}</a>
