...
>
    ...
    <!-- Modal Backdrop -->
    <div x-on:click="show = false" class="fixed inset-0 bg-black/75 dark:bg-zinc-900/80 transition-opacity"></div>

    <!-- Modal Content -->
    <div
        @class([
            'relative bg-white dark:bg-zinc-800 rounded-lg shadow-xl w-full max-h-[90vh] overflow-auto',
            'max-w-md' => $size === 'md',
            'max-w-lg' => $size === 'lg',
            'max-w-xl' => $size === 'xl',
            'max-w-sm' => $size === 'sm',
    ...
    >
        ...
        <!-- Modal Content -->
        <div class="p-4 text-zinc-800 dark:text-zinc-200">
            {{ $slot }}
        </div>
