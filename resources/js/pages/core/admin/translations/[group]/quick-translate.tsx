...
import { ArrowLeft, CheckCircle2, Languages, Save } from 'lucide-react';
...
export default function QuickTranslate() {
    const { group, groupName, currentKey, currentEnglish, remainingCount, availableLanguages, targetLanguage, progress } = usePage<PageProps>().props;
    ...
    useEffect(() => {
        // Reset translation and focus when currentKey changes
            ...
            <div className="space-y-6 px-6 py-4">
                <div className="flex items-center gap-4">
                    ...
                    <div className="flex-1">
                        <h1 className="text-2xl font-semibold">{t('admin.translations.quick_translate.title', 'Quick Translate')}</h1>
                    ...
                    <div className="mb-2 flex items-center justify-between">
                        ...
                        {progress.missing === 0 && (
                            <div className="flex items-center gap-2 text-green-500">
                                ...
                                <span className="text-sm font-medium">{t('admin.translations.all_complete', 'All translations complete!')}</span>
                            </div>
                        )}
                    </div>
                    <div className="h-2 w-full rounded-full bg-muted">
                        <div className="h-2 rounded-full bg-primary transition-all duration-300" style={{ width: `${progressPercentage}%` }} />
                    </div>
                </div>
                ...
                <div className="rounded-lg border border-border bg-card p-6">
                    <form onSubmit={handleSubmit} className="space-y-6">
                        <div className="space-y-2">
                            ...
                            <div className="rounded-md border border-border bg-muted p-3 font-mono text-sm break-all">{currentKey}</div>
                        </div>

                        <div className="space-y-2">
                            <Label>{t('admin.translations.quick_translate.base', 'English')}</Label>
                            <div className="min-h-[80px] rounded-md border border-border bg-muted p-4 text-sm">{currentEnglish}</div>
                        </div>
                        ...
                        <div className="flex justify-end gap-2">
                            ...
                            <Button type="submit">
                                ...
                                {remainingCount > 1 ? t('admin.translations.save_and_next', 'Save & Next') : t('admin.translations.save', 'Save')}
                            </Button>
                        </div>
                    </form>
                </div>
