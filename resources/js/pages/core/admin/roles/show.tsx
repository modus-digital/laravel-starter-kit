import { destroy, edit } from '@/routes/admin/roles';
                    ...
                    <div className="flex items-center justify-between">
                        <div className="flex items-center gap-4">
                            ...
                            <div>
                                ...
                                <p className="text-sm text-muted-foreground">{t('admin.roles.view_description')}</p>
                            </div>
                        </div>
                        <div className="flex gap-2">
                            <Button variant="outline" onClick={() => router.visit(edit({ role: role.id }).url)}>
                    ...
                    <div className="grid gap-6 md:grid-cols-2">
                                ...
                                {role.color && (
                                    <div>
                                        ...
                                        <div className="flex items-center gap-2">
                                            <div className="h-6 w-6 rounded border" style={{ backgroundColor: role.color }} />
                        ...
                        <Card>
                            <CardHeader>
                                ...
                                <CardDescription>{t('admin.roles.permissions_count', { count: role.permissions.length })}</CardDescription>
                            </CardHeader>
                            <CardContent>
                                {role.permissions.length === 0 ? (
                                    <p className="py-8 text-center text-sm text-muted-foreground">{t('admin.roles.no_permissions')}</p>
                                ) : (
                                    ...
                                    {activities.data.map((activity) => (
                                        <div key={activity.id} className="flex items-start space-x-4 text-sm">
                                            <div className="flex-1 space-y-1">
                                                <p className="text-sm leading-none font-medium">{String(renderActivityDescription(activity))}</p>
