...
export const GanttContentHeader: FC<GanttContentHeaderProps> = ({ title, columns, renderHeaderItem }) => {
    ...
    return (
        <div className="sticky top-0 z-20 grid w-full shrink-0 bg-card/80 backdrop-blur-sm" style={{ height: 'var(--gantt-header-height)' }}>
            <div>
                <div
            ...
            >
                {Array.from({ length: columns }).map((_, index) => (
                    <div className="shrink-0 border-b border-border/50 py-1 text-center text-xs" key={`${id}-${index}`}>
...
export const GanttSidebarHeader: FC = () => (
    <div
        className="sticky top-0 z-10 flex shrink-0 items-end justify-between gap-2.5 border-b border-border bg-card/80 p-2.5 text-xs font-medium text-muted-foreground backdrop-blur-sm"
...
export const GanttSidebarGroup: FC<GanttSidebarGroupProps> = ({ children, name, className }) => (
    <div className={className}>
        <p className="w-full truncate p-2.5 text-left text-xs font-medium text-muted-foreground" style={{ height: 'var(--gantt-row-height)' }}>
            {name}
        </p>
...
export const GanttSidebar: FC<GanttSidebarProps> = ({ children, className }) => (
    <div
        className={cn('sticky left-0 z-30 h-max min-h-full overflow-clip border-r border-border bg-card/80 backdrop-blur-md', className)}
    ...
    return (
        ...
        >
            <button
                ...
                type="button"
            >
                <PlusIcon className="pointer-events-none text-muted-foreground select-none" size={16} />
            </button>
        </div>
    );
};
...
export const GanttColumn: FC<GanttColumnProps> = ({ index, isColumnSecondary }) => {
    ...
    return (
        ...
        <div
            className={cn('group relative h-full overflow-hidden', isColumnSecondary?.(index) ? 'bg-muted/20' : '')}
...
export const GanttCreateMarkerTrigger: FC<GanttCreateMarkerTriggerProps> = ({ onCreateMarker, className }) => {
    ...
    return (
        <div className={cn('group pointer-events-none absolute top-0 left-0 h-full w-full overflow-visible select-none', className)} ref={mouseRef}>
            <div
...
export const GanttFeatureRow: FC<GanttFeatureRowProps> = ({ features, onMove, children, className }) => {
    ...
    for (const feature of sortedFeatures) {
        let subRow = 0;
        const effectiveEndAt = feature.endAt ?? farFuture;
...
export const GanttProvider: FC<GanttProviderProps> = ({ zoom = 100, range = 'monthly', onAddItem, children, className }) => {
    ...
    const hasInitialFocusScroll = useRef(false);
    ...
    useEffect(() => {
        if (hasInitialFocusScroll.current) {
            return;
        }
        ...
        const focusDate = addDays(new Date(), -3);

        const offset = getOffset(focusDate, timelineStartDate, {
            zoom,
            range,
            columnWidth,
            sidebarWidth,
        ...
        hasInitialFocusScroll.current = true;
    }, [columnWidth, headerHeight, onAddItem, range, rowHeight, setScrollX, sidebarWidth, timelineData, zoom]);
    ...
    return (
        ...
        >
            <div
                className={cn(
                    'gantt relative grid h-full w-full flex-none overflow-auto rounded-lg border border-border bg-card select-none',
                    range,
                    className,
                )}
                ref={scrollRef}
